...
class SugenoPDController(Node):
    def __init__(self):
        ...
        # Membresías para la derivada del error
        ...
        # Chingo de matriz de reglas
        # Formato: ('Etiqueta_Error', 'Etiqueta_Derivada') : (Velocidad_Lineal, Velocidad_Angular)
        self.reglas = {
            # error 0 (El robot está en el centro)
            ('z', 'z'):  (0.20, 0.0),    #Máxima velocidad pq está en el centro.
            ('z', 'lp'): (0.18, 0.010),  # En el centro, pero moviéndose a la derecha tons pequeña corrección izq.
            ('z', 'hp'): (0.15, 0.15),  # En el centro, pero derrapando rápido a la derecha tons freno preventivo.
            ('z', 'ln'): (0.18, -0.10),  
            ('z', 'hn'): (0.15, -0.15),   
            ...
            ('lp', 'hp'): (0.05, 0.25), # Freno casi total, giro máximo si se desvía rapido.
            ('lp', 'ln'): (0.18, 0.0),   #desviado pero acercandose, no girar mucho
            ('lp', 'hn'): (0.15, -0.10),  # Desviado, pero regresando muy rapido: Frena el giro para no pasarte.

            # muy a la derecha
            ('hp', 'z'):  (0.05, 0.25), # Muy desviado: Frena y gira fuerte.
            ('hp', 'lp'): (0.0, 0.25),  # Muy desviado y alejándose: Freno lineal, giro máximo.
            ('hp', 'hp'): (0.0, 0.25),  # se va y se aleja, frena y gira
            ('hp', 'ln'): (0.08, 0.15), # Muy desviado, pero ya empezó a regresar: Relaja el giro.
            ('hp', 'hn'): (0.10, 0.0),   #desviado pero acercandose, no girar 

            # ligero a la izqierda
        ...
        # Nombres de las etiquetas para iterar
    ...
    def error_callback(self, msg):
        #Obtener Error y calcular Derivada
        e = np.clip(msg.data, -1.0, 1.0)
        
        # Derivada, luego multiplicamo por un factor (3.0) 
        ...
        # 2. Fuzzificación: Obtener grados de pertenencia (suma ponderada)
        ...
        for l_e in self.labels:
            for l_de in self.labels:
                # El peso de la regla es el mínimo entre la certeza del Error y de la Derivada 
